...
class SimpleFeeder:
    ...
    def shuffle_files(self):
        ...
        self.file_set_names = {''.join(
            map(lambda data_file: data_file.name, files)): files for files in file_sets}
    ...
    class InnerSequencer(keras.utils.Sequence):
        ...
        def update_file_sets(self, file_sets):
            self.file_sets = [self.outer.MultiFilesAccessor(
                files, self.data_accessor, self.batch_size) for files in file_sets]
            self.cum_counts = np.cumsum(
                list(map(lambda fs: fs.get_batch_count(), self.file_sets)))
        ...
        def __getitem__(self, index):
            ...
            for i, j in enumerate(range(items_from, items_to)):
                ...
                if i < 1:
                    pass
                    # print("i =", i, " j =", j, " file_set_index =", file_set_index," file_index =", file_index, " item_index =", item_index)
    ...
    class MultiFilesAccessor:
        def __init__(self, files, data_accessor, batch_size):
            ...
            self.data_accessor = data_accessor
            self.cum_count = np.cumsum(
                [data_accessor.get_len(file) for file in files])
        ...
        def __getitem__(self, idx):
            ...
            index_in_file = item_index + \
                self.data_accessor.get_offset(self.files[file_index])
